...
- **Target (revised 2026-08-10):** 200-400 datapoints/entity (down from 1,000) across roughly 1,000 curated creators (up from ~15) — a deliberate breadth-over-depth pivot. Rationale: GAIL's biggest unresolved blocker for 5+ rounds has been zero real collaboration edges, which more creators (more chances at real overlaps) addresses more directly than deeper per-creator data does. "Entity" definition from the original target unchanged (one curated creator, datapoints summed across platforms) — only the numbers moved.
- **New process (refined 2026-08-10): identify → curate → deepen, not scrape-first.** (1) Identify all ~1,000 creator *candidates* in one broad pass (name/handle/category only, cheap) — not batched at this stage. Push all of them to a Google Sheet at once. (2) Curation is fully async: the user reviews at their own pace, marking approve/reject per row; this human review is the primary quality gate at this scale, replacing exhaustive per-handle verification (which doesn't scale from ~15 to ~1,000). Rejections trigger automatic backfill — Track A adds fresh replacement candidates so the reviewable pool never runs dry. (3) Deepening is gated on the user's explicit signal, per batch of 100 approved creators — not auto-triggered. Within a batch: Instagram and YouTube run concurrently (no shared resource), Reddit runs after Instagram releases the browser tab lease (see the tab-lease-contention finding below). Expect each 100-batch's Instagram leg to span multiple rounds, not complete in one. Google Sheets sync needs real setup (a service account, a one-way-push-plus-status-column-readback protocol) — not assumed to exist.
- **Platform priority (revised 2026-08-10): Instagram first.** Most real sponsorship signal found so far (e.g., Kohli/Agilitas) came from Instagram captions, not YouTube — campaigns concentrate there. Trade-off to keep in view: Instagram is also the most fragile/slowest platform (session-based, confirmed to serialize with Reddit via tab-lease contention, prior comment-truncation issues) — prioritizing it will likely slow raw throughput even as creator count rises.
- **Coverage (widened 2026-08-10):** athletes, fitness/athletics creators, team/league accounts (IPL teams, football clubs, cricket teams including local clubs, not just national/global ones) — team accounts are a deliberate addition, since one team post tagging multiple players is a clean, natural collaboration edge, directly attacking the zero-real-edges blocker. India-preferred, not a hard restriction, per the earlier target-list-diversification decision. 5k+ followers, last 6 months (rolling window) as originally scoped.
...
- **Inductive setting (revised 2026-08-09):** originally planned as a Weeks 11-13 swap to GraphSAGE for inductive new-node embedding. Track B verified (structurally and empirically — a trained model applied without retraining to a graph 3x larger worked correctly) that GAT already has this inductive property; `SAGEConv` also turned out to have zero `edge_attr` support, making the swap non-trivial anyway. **Provisionally staying on GAT for production, skipping the GraphSAGE swap** — still needs re-validation at real-data scale (the test above used synthetic data) before this is fully closed out. Periodic fine-tuning as the graph evolves should still be planned for regardless of backbone.
