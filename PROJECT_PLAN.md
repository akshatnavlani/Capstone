...
- **New process (2026-08-10): identify → curate → deepen, not scrape-first.** (1) Identify creator *candidates* (name/handle/category only, cheap) via search — not just recalling known names. (2) Push candidates to a Google Sheet for human review; the user removes anything irrelevant. This human review step is now the primary quality gate, replacing exhaustive per-handle verification for every candidate — that verification doesn't scale to ~1,000 creators the way it worked for 15. (3) Only for approved creators: basic profile enrichment, then full content scraping. Google Sheets sync needs real setup (a service account, a defined one-way-push-plus-status-column-readback protocol) — not assumed to exist.
