...
- **Target:** minimum 1,000 datapoints/entity (posts, comments, metrics, thumbnails, timestamps); influencer count maximized based on actual Month 1-2 scraping velocity — treat the count as a rolling target, checked at the Week 4 and Week 8 checkpoints, not a fixed number decided today. **Definition (clarified 2026-08-10):** "entity" = one curated target creator on the deliberate target list (e.g., Cristiano Ronaldo, Virat Kohli) — the 1,000 floor is measured per named creator, summed across all their platform content. It does NOT mean any scraped profile row (e.g., an incidental commenter who never becomes a target) and does NOT mean total datapoints across the whole dataset.
